...
def spectrogram_compare(sample, beat_times, spectrograms):
    _, sample_t, sample_sp = spectrogram(sample.synthesize())
    diffs = np.array([])
    for t, sp in zip(beat_times, spectrograms):
        t_min = min([t.size, sample_t.size])
        if (sample_t[:t_min] == t[:t_min]).all():
            diffs = np.append(diffs, (sample_sp[:, :t_min] - sp[:, :t_min]).mean())
    return diffs.mean()
...
class NetworkAPI():
    ...
    def train_loop(self, iterations, verbose_every_iteration = 10000, generate_every_iteration = 100, song_len=1000):
        ...
        generations = int(iterations/generate_every_iteration)
        gen_start = 0
        for iteration in range(iterations):
            ...
            if iteration % generate_every_iteration == 0:
                ...
                # Calculate mean spectrogram differences
                self.mean_spectrogram_diff.append(spectrogram_compare(sample_midi, self.DataAPI.get_beat_times(), self.DataAPI.get_spectrograms()))
            ...
            if loss < best_loss:
                state = {
                            ...
                            'mean_spectrogram_diff': self.mean_spectrogram_diff
                        }
    ...
    def load_checkpoint(self):
        ...
        self.mean_spectrogram_diff=checkpoint['mean_spectrogram_diff']
